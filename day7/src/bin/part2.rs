...
impl Hand<'_> {
    fn from(x: (&str, usize)) -> Hand {
        ...
        let card_map_jokers: &u32 = &card_map.remove(&'J').unwrap_or_else(|| 0_u32);

        if card_map_jokers == &5 {
            return Hand {
                cards: x.0,
                bet: x.1,
        ...
        let mut card_map_values = card_map.values().map(|x| *x).into_iter().collect::<Vec<u32>>();
        card_map_values.sort();

        let last_card_count = card_map_values.pop().unwrap();
        card_map_values.push(last_card_count + card_map_jokers);
